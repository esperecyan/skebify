...
async function upsertButton(twitterId: string, data: SkebUserResponse) {
  const headerItems = getParent()
  ...
  const anchorHTML = buildSkebLink(
    twitterId,
    data.screen_name,
...
function getTwitterId(scriptNode: HTMLScriptElement): string {
    ...
    for (const node of m.addedNodes) {
      if (
        ...
        if (scriptNode.getAttribute("type") === "application/ld+json") {
          const twitterId = getTwitterId(scriptNode)
          const r = await chrome.runtime.sendMessage({
            id: twitterId
          })
          await upsertButton(twitterId, r)
          break
        }
      }
    }
...
// Prevent link from being removed when navigating to tab
new MutationObserver((mutations) => {
  for (const m of mutations) {
    for (const node of m.removedNodes) {
      if (node.nodeType === Node.ELEMENT_NODE) {
        const skebLink = (node as HTMLElement).querySelector<HTMLAnchorElement>(
          ".skeb"
        )
        if (skebLink !== null) {
          if (
...
}).observe(document.querySelector<HTMLDivElement>("main > div"), {
  childList: true,
  subtree: true
})
