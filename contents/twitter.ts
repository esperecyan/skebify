...
const twitterIdSkebUserResponseMap: Map<string, SkebUserResponse> = new Map()

async function upsertButton() {
  const headerItems = document.querySelector<HTMLDivElement>(
  ...
  const scriptNode = document.querySelector<HTMLScriptElement>(
    '[data-testid="UserProfileSchema-test"]'
  )
  const userProfile = JSON.parse(scriptNode.text)
  const twitterId = userProfile.mainEntity.identifier

  if (headerItems.dataset.twitterId === twitterId) {
    return
  }

  headerItems.dataset.twitterId = twitterId

  // remove existing skebify element
  headerItems.getElementsByClassName("skeb")[0]?.remove()

  const data: SkebUserResponse = twitterIdSkebUserResponseMap.has(twitterId)
    ? twitterIdSkebUserResponseMap[twitterId]
    : await chrome.runtime.sendMessage({ id: twitterId })
  if (data === null) {
    return
  }
...
new MutationObserver((mutations, observer) => {
  ...
  new MutationObserver(upsertButton).observe(main, { childList: true })

  // detect transition to another user's profile
  new MutationObserver(upsertButton).observe(document.head, { childList: true })
}).observe(document.getElementById("react-root"), {
  childList: true,
  subtree: true
})
